...
from PIL import Image  # 导入 PIL 库用于读取图像文件
import cv2
...
def convert_all_labels(input_dir, output_dir, image_shape):
    """
    批量转换目录下所有的.tif标签文件为YOLO格式
    :param input_dir: 输入的标签目录，包含.tif格式标签
    :param output_dir: 输出的标签目录，保存YOLO格式标签
    :param image_shape: 图像的尺寸 (H, W)
    """
    tif_files = Path(input_dir).rglob("*.jpg")  # 遍历标签文件夹下所有.tif文件

    for tif_file in tif_files:
        gt2D = np.array(Image.open(tif_file))  # 加载标签数据
        gt2D[gt2D < 128] = 0
        gt2D[gt2D > 128] = 1
        gt2D = cv2.resize(gt2D, image_shape, interpolation=cv2.INTER_NEAREST)
        #gt2D=gt2D/255
        
        img_name = tif_file.stem  # 获取文件名（不带扩展名）
...
# 示例用法
input_dir = '/hpc2hdd/home/xingmu/MedSAM/data/polyp_test/masks'  # 包含 .tif 标签的文件夹
output_dir = '/hpc2hdd/home/xingmu/MedSAM/data/dataset2/labels/val'  # 输出 YOLO 标签的文件夹
