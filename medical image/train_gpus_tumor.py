...
import monai
from segment_anything3 import sam_model_registry
...
class NpyDataset(Dataset):
    def __init__(self, data_root):
        ...
        self.gt_path_files = sorted(
            glob.glob(join(self.gt_path, "**/*.jpg"), recursive=True)
        )
        self.gt_path_files = [
            file
    ...
    def __getitem__(self, index):
        ...
        #)  # (1024, 1024, 3)
        img_1024=cv2.imread(join(self.img_path, img_name))
        img_1024 = cv2.resize(img_1024, (1024,1024))
        ...
        gt=Image.open(self.gt_path_files[index])
        gt=np.array(gt)/255
...
parser.add_argument(
    "-checkpoint", type=str, default="sam_vit_b_01ec64.pth"
)
...
def main():
    ...
    for name, module in medsam_model.named_modules():
        if "mask_decoder" in name or "prompt" in name or "adapter" in name or "lora_q" in name or "lora_k" in name or "lora_v" in name:
